...
embedding_kmers = []
with open("/home/ubuntu/search_for_kmers_python/kmersDimension1024") as f:
...
if __name__ == '__main__':
    ...
    sequences = sc.textFile(sequences_file).filter(lambda x: not x.startswith('>')).take(10)
    sequences_par = sequences.map(embed).reduceByKey(lambda a, b: a).saveAsTextFile("/spark_results/dim1024")
    sc.stop()
